...
android_setup(struct usb_gadget *gadget, const struct usb_ctrlrequest *c)
{
	...
	if (value < 0)
        ...
        if (connect2pc == 0) {
          connect2pc = 1;
          switch_set_state(&dev->sw_connect2pc, 1);
        }
	spin_lock_irqsave(&cdev->lock, flags);
	if (!dev->connected) {
		dev->connected = 1;
...
static void android_disconnect(struct usb_gadget *gadget)
{
	struct android_dev *dev = _android_dev;
        if (connect2pc == 1) {
          connect2pc = 0;
          switch_set_state(&dev->sw_connect2pc, 0);
        }
	dev->connected = 0;
	schedule_work(&dev->work);
...
static int __init init(void)
{
	...
	_android_dev = dev;
	dev->sw_connect2pc.name = "usb_connect2pc";
	err = switch_dev_register(&dev->sw_connect2pc);
	if (err < 0)
          return err;
